...
const STATUS_BADGE: Record<string, 'teal' | 'forest' | 'amber' | 'brick' | 'neutral'> = {
  live:      'teal',
  published: 'forest',
  draft:     'neutral',
  archived:  'neutral',
...
const REG_BADGE: Record<string, 'forest' | 'amber' | 'brick' | 'neutral'> = {
  confirmed: 'forest',
  approved:  'forest',
  pending:   'amber',
  cancelled: 'brick',
      ...
      <div className={styles.kpiSection}>
        <div className={styles.kpiGrid}>
          ...
          <KpiCard
            ...
            value={String(confirmedSpeakers)}
            color="i"
          />
        </div>
      </div>
              ...
              {events.map((ev, i) => {
                ...
                return (
                  <tr key={ev.id} className={i < events.length - 1 ? styles.tr : styles.trLast}>
                    ...
                    <td className={styles.td}>
                      <Badge variant={STATUS_BADGE[ev.status] ?? 'neutral'}>{ev.status}</Badge>
                    </td>
            ...
            {allRegs.slice(0, 8).map(reg => {
              ...
              return (
                <div key={reg.id} className={styles.regRow}>
                  <Avatar name={`${att.firstName} ${att.lastName}`} size="sm" />
                  ...
                  <Badge variant={REG_BADGE[reg.status] ?? 'neutral'}>{reg.status}</Badge>
                </div>
              )
            })}
          </div>
